...
class FlagsScreen(Screen):
    ...
    def on_country_selected(self, country_name):
        try:
            ...
            self.selected_country = country  # Store the selected country
            app = App.get_running_app()
            app.country_code = country['dial_code'].replace('+', '')
            app.selected_country = country['name']  # Store country name
            app.selected_country_data = country  # Store full country data

            # Get the selected_flag widget from ids
            if hasattr(self.ids, 'selected_flag'):
                selected_flag = self.ids.selected_flag
                
                # Try to load cached flag first
                cached_flag = self.get_cached_flag_path(country['code'])
                if os.path.exists(cached_flag):
                    selected_flag.source = cached_flag
                    return
